...
| Prompt drift on a silent system prompt edit, same behavioral risk as a model change but easy to dismiss as "just words" | A hash of the exact rendered system prompt (after vault-specific variables are substituted) is stored alongside modelId in every DecisionRecord, so any prompt change that skipped a Paper Vault re-validation pass is just as visible and auditable as an unvalidated model change, same discipline applied to both |
