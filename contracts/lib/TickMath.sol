...
/// @dev Ported from Uniswap V3 core (GPL-2.0-or-later), adapted to solc 0.8.x (unchecked blocks replace the
/// original's reliance on 0.7.x's default wrapping arithmetic in the exact spots the original relied on it,
/// verified line-for-line against the canonical implementation, not reinvented). Vendored directly since no
/// Uniswap npm package is installed in this repo.
library TickMath {
    ...
    function getTickAtSqrtRatio(uint160 sqrtPriceX96) public pure returns (int24 tick) {
        unchecked {
            ...
            assembly {
                let f := shl(7, gt(r, 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF))
                msb := or(msb, f)
                r := shr(f, r)
            }
            assembly {
                let f := shl(6, gt(r, 0xFFFFFFFFFFFFFFFFFFFFFFFF))
                msb := or(msb, f)
                r := shr(f, r)
            }
            assembly {
