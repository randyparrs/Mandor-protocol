...
/// @notice A LEANER SIBLING of contracts/MandateVault.sol, not a fork of an
/// unrelated design: same ERC-4626 custody model, same
/// propose/pre-check/confirm/execute pipeline, same shared-source-across-
/// versions philosophy, just without any Uniswap-V3-style LP capability
/// (contracts/MandateVault.sol's own LP_OPEN/LP_INCREASE/LP_DECREASE/
/// LP_COLLECT/LP_CLOSE machinery, used only by v3). Exists purely because
/// contracts/MandateVault.sol sat only 169 bytes under EIP-170's
/// 24,576-byte deployable-contract limit even before v6's performance-fee
/// addition (see foundry.toml's own optimizer comment): adding the fee
/// mechanism there pushed it 482 bytes over, even at this project's most
/// aggressive available optimizer settings, and extracting small pieces of
/// NEW logic to an external library was measured to cost MORE bytecode
/// than it saved (call-encoding overhead exceeds the logic it would
/// replace for anything this small, the same real finding already
/// documented in MandateVault.sol's own _tickCumulativeDelta comment for
/// LP math). Properly extracting v3's LP mechanism into its own registry
/// contract (the same pattern LendingPositionRegistry already proves for
/// cross-chain lending) is the correct long-term fix, deliberately
/// deferred to a dedicated future session rather than rushed here.
///
/// @notice Used starting with v6 (cross-chain lending + a performance fee)
/// and any future version that has no need for the LP mechanism. v1-v5
/// keep deploying from contracts/MandateVault.sol unchanged; this file
/// does not affect any of their already-deployed instances in any way.
///
...
contract MandateVaultLending is ERC4626, IAutoPausePayer, IStaleWithdrawalBountyPayer, ReentrancyGuard {
    ...
    /// @dev Kept for ABI compatibility with contracts/MandateVault.sol's own
    /// executeDecision (same executor/keeperServiceV4.ts drives both v5 and
    /// this contract's v6, see that file's own "INTENTIONAL FORK" doc
    /// comment), never actually acted on here: this contract has no LP
    /// mechanism at all, see this file's own top-of-file doc comment for
    /// why. The keeper always supplies an empty leg (pool == address(0),
    /// tokenId == 0) for this vault, same harmless-when-unused convention
    /// v1/v2 already use for v3/v4-only ConstructorLimits fields.
    struct LpLeg {
        address pool;
        uint24 fee;
        int24 tickLower;
    ...
    /// No LP positions to value here (this contract has no LP mechanism at
    /// all, see this file's own top-of-file doc comment); cross-chain
    /// lending positions are the one real external valuation this contract
    /// performs, delegated entirely to LendingPositionRegistry.
    ...
    function executeDecision(
        ...
        LpLeg calldata, // never acted on, kept for ABI compatibility, see below
        ...
        // lpLeg is never acted on: this contract has no LP mechanism, see
        // this file's own top-of-file doc comment. Kept only as an unused
        // parameter for ABI compatibility with the keeper that also drives
        // contracts/MandateVault.sol-based vaults.
        ...
        returns (IVaultPolicy.VaultState memory)
    {
        ...
        return IVaultPolicy.VaultState({
            ...
            prices: prices,
            // Always empty: this contract has no LP mechanism at all, see
            // this file's own top-of-file doc comment. VaultPolicy's LP
            // checks simply never find anything to evaluate here.
