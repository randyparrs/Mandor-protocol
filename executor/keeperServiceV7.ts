// The v7 (LP yield vault, WUSDC/EURC) counterpart to
// executor/keeperServiceV4.ts. Holds TWO real signing keys inherited from
// that fork (KEEPER_PRIVATE_KEY, and the deliberately SEPARATE
// ARBITRUM_SEPOLIA_KEEPER_PRIVATE_KEY) even though v7 itself has no
// cross-chain lending strategy today -- contracts/MandateVaultLp.sol was
// built from contracts/MandateVaultLending.sol's base (which does carry
// the full BridgeLeg/CCTP machinery, unused but harmless for v7, see that
// contract's own top-of-file comment), so this fork's bridge-leg code
// paths stay dormant, not deleted, matching that same "harmless when
// unused" convention.
//
// INTENTIONAL FORK, READ BEFORE FIXING A BUG HERE (2026-07-27): this file
// is a deliberate, separate copy of executor/keeperServiceV4.ts, created
// specifically because contracts/MandateVaultLp.sol has NO positionManager()
// getter on the vault itself -- that state now lives on
// contracts/LpPositionRegistry.sol instead (see that contract's own
// top-of-file comment for why: real, measured EIP-170 pressure forced the
// LP mechanism out of the vault and into its own satellite contract for
// v7). keeperServiceV4.ts's own buildLpLeg reads positionManager directly
// from the vault address, which is correct for v3 (contracts/MandateVault.sol
// has that field itself) but would silently resolve to the wrong address
// (or revert) for a v7-shaped vault. Rather than adding a v7-aware branch
// inside keeperServiceV4.ts -- the exact module currently driving v4/v5/v6's
// real, live, scheduled decision cycle -- this is a full, separate fork,
// same "new version = new deployment, never touch a live one" principle
// already applied to the contracts themselves and to keeperServiceV4.ts's
// own split from executor/keeperService.ts.
//
// Consequence: a security-relevant fix discovered in ANY of these three
// files (executor/keeperService.ts, executor/keeperServiceV4.ts, this one)
// -- e.g. a real swap/LP-leg sizing bug, a missing revert check, an
// alerting gap -- does NOT automatically apply to the other two. Manually
// check the other two files whenever you fix something security-relevant
// here.
//
// The one real code change from keeperServiceV4.ts: buildLpLeg below reads
// lpRegistry() from the vault first, then positionManager() from THAT
// registry address, instead of reading positionManager() from the vault
// directly (see that function's own comment for the exact diff).
...
async function buildLpLeg(
  ...
  // v7 only real difference from keeperServiceV4.ts's own buildLpLeg:
  // contracts/MandateVaultLp.sol has NO positionManager() getter at all --
  // that state lives on contracts/LpPositionRegistry.sol instead (real
  // EIP-170 pressure forced the LP mechanism out of the vault, see that
  // contract's own top-of-file comment). Resolve lpRegistry() from the
  // vault first, then positionManager() from THAT address.
  const lpRegistryAddress = await publicClient.readContract({ address: vaultAddress, abi: [{ type: "function", name: "lpRegistry", stateMutability: "view", inputs: [], outputs: [{ type: "address" }] }] as const, functionName: "lpRegistry" });
  if (lpRegistryAddress === ZERO_ADDRESS) throw new Error("Vault has no lpRegistry set, cannot build an LP leg.");
  const positionManager = await publicClient.readContract({ address: lpRegistryAddress, abi: [{ type: "function", name: "positionManager", stateMutability: "view", inputs: [], outputs: [{ type: "address" }] }] as const, functionName: "positionManager" });
  const deadline = BigInt(Math.floor(Date.now() / 1000)) + SWAP_DEADLINE_BUFFER_SECONDS;
...
/// constants, see KeeperServiceV7Config's own attestationPollIntervalMs/
...
export interface KeeperServiceV7Config {
  publicClient: PublicClient;
  vaultAddress: `0x${string}`;
  assets: KnownAsset[];
...
/// all.
export class KeeperServiceV7 implements Executor {
  ...
  constructor(private readonly config: KeeperServiceV7Config) {
