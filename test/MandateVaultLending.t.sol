...
import {MandateVaultLending} from "../contracts/MandateVaultLending.sol";
...
import {MockLendingRegistryForFee} from "../contracts/test/MockLendingRegistryForFee.sol";
import {IVaultPolicy} from "../contracts/interfaces/IVaultPolicy.sol";

/// @notice Deliberately the SAME test bodies as test/MandateVault.t.sol,
/// run against MandateVaultLending instead: proves the LP-free variant
/// behaves identically to contracts/MandateVault.sol for every piece of
/// shared, non-LP functionality (deposits, pause gating, drawdown, bounty
/// payouts, ledger accounting, router allowlist, sweep dust, capital
/// registry cap), not just "still compiles" or "tests don't fail on
/// something new." See contracts/MandateVaultLending.sol's own
/// top-of-file doc comment for why this variant exists.
contract MandateVaultLendingTest is Test {
    ...
    MandateVaultLending internal vault;
    VaultPolicy internal policy;

    address internal user1 = address(0xA11CE);
    address internal user2 = address(0xB0B);

    // Performance-fee tests only: `factory` is always address(this) in
    // this file's setUp (see the vault's own constructor call below), so
    // this test contract IS the "factory" _accrueFee queries for
    // protocolTreasury via IVaultFactory -- same wiring pattern as the
    // real VaultFactory, just satisfied directly here since no real
    // VaultFactory instance is deployed in these unit tests.
    ...
    function setUp() public {
        ...
        vault = new MandateVaultLending(IERC20(address(usdc)), address(roles), address(router), "Mandate USDC Vault", "mUSDC", otherAssets, address(this), address(0));
    ...
    /// convention MandateVaultLending.executeDecision itself uses.
    function _emptyLpLeg() internal pure returns (MandateVaultLending.LpLeg memory) {
        return MandateVaultLending.LpLeg({
            pool: address(0),
            fee: 0,
            tickLower: 0,
            tickUpper: 0,
    ...
    /// call," same convention MandateVaultLending.executeDecision itself uses
    /// (mirrors _emptyLpLeg's own pool == address(0) convention).
    function _emptyBridgeLeg() internal pure returns (MandateVaultLending.BridgeLeg memory) {
        return MandateVaultLending.BridgeLeg({chainId: 0, amount: 0, positionId: 0, cctpDestinationDomain: 0, maxFee: 0});
    }
    ...
    function testFuzz_executeDecisionRevertsWheneverValidateDecisionWouldReject(uint256 depositAmount) public {
        ...
        MandateVaultLending.SwapLeg[] memory swaps = new MandateVaultLending.SwapLeg[](0);
    ...
    /// @dev sweepDust is the one place MandateVaultLending ever reads a live
    ...
    // -------------------------------------------------------------------
    // Performance fee (deliberately a separate vault+policy pair from
    // this file's shared vault/policy above, which always has
    // performanceFeeBps: 0, see setUp).
    // -------------------------------------------------------------------

    function _depositTo(MandateVaultLending v, address user, uint256 amount) internal {
        usdc.mint(user, amount);
        vm.startPrank(user);
    ...
    function _deployFeeVault(uint256 feeBps) internal returns (MandateVaultLending feeVault, VaultPolicy feePolicy, MockLendingRegistryForFee registry) {
        feeVault = new MandateVaultLending(IERC20(address(usdc)), address(roles), address(router), "Fee Vault", "mFEE", new address[](0), address(this), address(0));
        ...
        feeVault.setPolicy(address(feePolicy));

        registry = new MockLendingRegistryForFee();
        feeVault.setLendingRegistry(address(registry));
    }

    /// @dev The seed deposit itself never charges a fee (supply is 0 when
    /// _accrueFee runs, before _deposit mints anything). The real risk case
    /// is the SECOND ever call: without _accrueFee's lazy high-water-mark
    /// initialization, this would otherwise see a high-water-mark still at
    /// its default 0 and treat the vault's entire existing NAV as "profit,"
    /// charging a fee on a depositor's own principal.
    function test_performanceFee_zeroOnPureDeposit() public {
        (MandateVaultLending feeVault, , ) = _deployFeeVault(1000); // 10%
        _depositTo(feeVault, user1, 1_000_000e18);
        _depositTo(feeVault, user2, 500_000e18);

        assertEq(feeVault.balanceOf(feeRecipient), 0, "no yield occurred, fee recipient must hold zero shares");
        assertGt(feeVault.feeHighWaterMarkPricePerShare(), 0, "the high-water-mark must be initialized by now, just never taxed at initialization");
    }

    /// @dev Independently recomputes the exact formula _accrueFee itself
    /// uses (from captured pre-state), then asserts the contract's real
    /// post-state matches -- confirms the implementation matches the
    /// documented spec, not just "some fee was minted."
    function test_performanceFee_mintsCorrectSharesOnRealYield() public {
        (MandateVaultLending feeVault, , MockLendingRegistryForFee registry) = _deployFeeVault(1000); // 10%
        _depositTo(feeVault, user1, 1_000_000e18);
        feeVault.accrueFee(); // lazy baseline init, must not itself charge a fee
        assertEq(feeVault.balanceOf(feeRecipient), 0, "baseline initialization must never itself charge a fee");

        registry.setTotalValueUSDC(100_000e18); // genuine new yield

        uint256 supplyBefore = feeVault.totalSupply();
        uint256 navBefore = feeVault.totalAssets();
        uint256 unit = 10 ** feeVault.decimals();
        uint256 hwmBefore = feeVault.feeHighWaterMarkPricePerShare();
        uint256 priceBefore = feeVault.convertToAssets(unit);

        uint256 expectedProfitUSDC = ((priceBefore - hwmBefore) * supplyBefore) / unit;
        uint256 expectedFeeUSDC = (expectedProfitUSDC * 1000) / 10_000;
        uint256 expectedFeeShares = (expectedFeeUSDC * supplyBefore) / (navBefore - expectedFeeUSDC);

        feeVault.accrueFee();

        assertEq(feeVault.balanceOf(feeRecipient), expectedFeeShares, "fee recipient must receive exactly the independently-computed fee shares");
    }

    /// @dev The specific scenario flagged explicitly for its own dedicated
    /// test: price per share rises (fee charged once), falls (no fee, and
    /// the high-water-mark never moves down), then recovers back to
    /// EXACTLY the same prior peak without exceeding it (still no fee --
    /// the same gain is never charged twice). Only a genuinely new peak
    /// above the prior high-water-mark charges a fee again.
    function test_performanceFee_noDoubleChargeOnRecoveryToSamePeak() public {
        (MandateVaultLending feeVault, , MockLendingRegistryForFee registry) = _deployFeeVault(1000); // 10%
        _depositTo(feeVault, user1, 1_000_000e18);
        feeVault.accrueFee(); // lazy baseline init

        // Rise to a new peak: genuine yield, fee charged once.
        registry.setTotalValueUSDC(100_000e18);
        feeVault.accrueFee();
        uint256 sharesAfterFirstFee = feeVault.balanceOf(feeRecipient);
        uint256 hwmAfterFirstFee = feeVault.feeHighWaterMarkPricePerShare();
        assertGt(sharesAfterFirstFee, 0, "genuine yield must charge a real fee");

        // Fall: a real loss (lending position reports less value).
        registry.setTotalValueUSDC(40_000e18);
        feeVault.accrueFee();
        assertEq(feeVault.balanceOf(feeRecipient), sharesAfterFirstFee, "a loss must never charge a fee");
        assertEq(feeVault.feeHighWaterMarkPricePerShare(), hwmAfterFirstFee, "high-water-mark must never decrease");

        // Recover back to EXACTLY the same previous peak, not exceeding
        // it: the specific case that must never be double-charged.
        registry.setTotalValueUSDC(100_000e18);
        feeVault.accrueFee();
        assertEq(feeVault.balanceOf(feeRecipient), sharesAfterFirstFee, "recovering to a prior peak, without exceeding it, must never charge a fee again on the same gain");
        assertEq(feeVault.feeHighWaterMarkPricePerShare(), hwmAfterFirstFee, "high-water-mark must stay exactly at the prior peak after a mere recovery");

        // Genuinely new gain, above the prior peak: must charge a fee
        // again, proving the mechanism isn't simply stuck forever.
        registry.setTotalValueUSDC(150_000e18);
        feeVault.accrueFee();
        assertGt(feeVault.balanceOf(feeRecipient), sharesAfterFirstFee, "a genuinely new peak above the prior high-water-mark must charge a fee");
    }
}
