// Deploys the real LendingPositionRegistry for the real v6 vault
// (0x10623f7715e8fD8D3C8F06A2000FbCEeA5002624, see docs/deployments.md's
// v6 section). No privileged role gates this constructor (just records the
...
//
// Reuses the same Arbitrum Sepolia keeper wallet already generated and
// wired for v4 (0xc5c828D0AC3e106C5006c4b62c3eb2405A5462b3, see
// scripts/generateArbitrumSepoliaKeeperWallet.ts and
// scripts/deployLendingPositionRegistryV4.ts): same off-chain
// executor/keeperServiceV4.ts identity, same Arbitrum Sepolia chain, just
// proposed as chainKeeper on a second, separate LendingPositionRegistry
// instance (one per vault) -- no reason to generate a new wallet for the
// same real-world keeper operator.
//
...
// (2-of-2), same flow already proven for v4. This script only deploys the
// registry and prints the exact calldata for those two Safe transactions.
//
// Run with: npx hardhat run scripts/deployLendingPositionRegistryV6.ts --network arcTestnet
import { network } from "hardhat";
import { getAddress, encodeFunctionData, type Hash } from "viem";

const VAULT_V6_ADDRESS = getAddress("0x10623f7715e8fD8D3C8F06A2000FbCEeA5002624");
const POLICY_V6_ADDRESS = getAddress("0xb259ab55F2690E38182223FE193f1b777fF7a600");
...
async function main() {
  ...
  const registry = await viem.deployContract(
    ...
    [VAULT_V6_ADDRESS, POLICY_V6_ADDRESS, MANDATE_ROLES_ADDRESS],
    { client: { wallet: admin } },
  );
  console.log(`LendingPositionRegistry (v6): ${registry.address}`);
  ...
  if (getAddress(liveVault) !== VAULT_V6_ADDRESS || getAddress(livePolicy) !== POLICY_V6_ADDRESS || getAddress(liveRoles) !== MANDATE_ROLES_ADDRESS) {
  ...
  console.log(`  To:       ${VAULT_V6_ADDRESS}`);
  console.log(`  Value:    0`);
